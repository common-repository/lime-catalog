...
class LMCTLG_Amount {
	...
	public static function lmctlg_format_amount_to_string($amount) {
	  ...
	  $decimal_separator  = sanitize_text_field( stripslashes( $lmctlg_currency_options['decimal_separator'] ) );
	...
	{ 	
	  ...
	  $thousand_separator = sanitize_text_field( stripslashes( $lmctlg_currency_options['thousand_separator'] ) );
	  $decimal_separator  = sanitize_text_field( stripslashes( $lmctlg_currency_options['decimal_separator'] ) );
	  $number_of_decimals = sanitize_text_field( stripslashes( $lmctlg_currency_options['number_of_decimals'] ) );
	  ...
	  if ( $thousand_separator == ',' and $decimal_separator == '.' ) {
		  ...
		  $amount = __('Error: Not well formed amount.', 'lime-catalog');
	  }
	...
	{ 
	  if ($amount >= 0) {  
		...
		$currencyspan = '<span class="lmctlg-currency-symbol">' . esc_html( $currencysymbol ) . '</span>';
		$amountspan   = '<span class="lmctlg-item-price">' . esc_html( $amount ) . '</span>';
	...
	{ 
	  if ($amount >= 0) {  
		...
		// use span - to identify for jquery updates
		$currencyspan = '<span class="lmctlg-currency-symbol">' . esc_html( $currency_symbol ) . '</span>';
		$amountspan   = '<span class="lmctlg-item-price">' . esc_html( $amount ) . '</span>';
